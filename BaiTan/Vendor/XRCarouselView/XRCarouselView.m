...
//图片边距
@property (nonatomic, assign) CGFloat imageBorder;
...
#pragma mark 代码创建
- (instancetype)initWithFrame:(CGRect)frame  imageBorder:(CGFloat)imageBorder {
...
-(void)setIndexLableText:(NSString *) textString{
    
    NSMutableAttributedString *string = [[NSMutableAttributedString alloc] initWithString:textString attributes: @{NSFontAttributeName: [UIFont fontWithName:@"PingFangSC-Regular" size: 9],NSForegroundColorAttributeName: [UIColor colorWithRed:255/255.0 green:255/255.0 blue:255/255.0 alpha:1.0]}];
    self.indexLable.attributedText = string;
}
...
-(UILabel *) indexLable{
    ...
    if (!_indexLable && !self.indexLableIfHiden) {
        _indexLable = [[UILabel alloc] initWithFrame:CGRectMake(10 + self.imageBorder, self.frame.size.height - 10 - 13 , 25, 13)];
        _indexLable.textColor = [UIColor whiteColor];
        _indexLable.backgroundColor = [UIColor colorWithRed:41.0/255.0 green:35/255.0 blue:54/255.0 alpha:0.5];
        _indexLable.layer.cornerRadius = 6.5;
        _indexLable.clipsToBounds = YES;
        _indexLable.textAlignment = NSTextAlignmentCenter;
        _indexLable.font = [UIFont systemFontOfSize:9.0];
...
- (void)setImageArray:(NSArray *)imageArray{
    ...
    [self.indexLable setHidden:(imageArray.count == 1)];
    ...
    [self setIndexLableText: [NSString stringWithFormat:@"1/%lu",(unsigned long)_images.count]];
    [self layoutSubviews];
}
...
- (void)setScrollViewContentSize {
    if (_images.count > 1) {
        ...
        self.currImageView.frame = CGRectMake(self.width * 2 + self.imageBorder, 0, self.width - self.imageBorder * 2, self.height);
        
        if (_changeMode == ChangeModeFade) {
            //淡入淡出模式，两个imageView都在同一位置，改变透明度就可以了
            _currImageView.frame = CGRectMake(self.imageBorder, 0, self.width - self.imageBorder * 2, self.height);
        ...
        self.currImageView.frame = CGRectMake(self.imageBorder, 0, self.width - self.imageBorder * 2, self.height);
        [self stopTimer];
    }
}
...
- (void)nextPage {
    if (_changeMode == ChangeModeFade) {
        ...
        [UIView animateWithDuration:1.5 animations:^{
            ...
            [self setIndexLableText:[NSString stringWithFormat:@"%lu/%lu",_nextIndex + 1,(unsigned long)_images.count]];
...
- (void)changeCurrentPageWithOffset:(CGFloat)offsetX {
    if (offsetX < self.width * 1.5) {
        ...
        [self setIndexLableText:[NSString stringWithFormat:@"%lu/%lu",index + 1,(unsigned long)_images.count]];

      
    } else if (offsetX > self.width * 2.5){
        self.pageControl.currentPage = (self.currIndex + 1) % self.images.count;
        [self setIndexLableText:[NSString stringWithFormat:@"%lu/%lu",((self.currIndex + 1) % self.images.count) + 1, (unsigned long)_images.count]];

    } else {
        self.pageControl.currentPage = self.currIndex;
        [self setIndexLableText:[NSString stringWithFormat:@"%lu/%lu",self.currIndex + 1,(unsigned long)_images.count]];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
  ...
  if ([self.delegate respondsToSelector:@selector(carouselViewScrolling)]){
      [self.delegate carouselViewScrolling];
  }
    ...
    if (offsetX < self.width * 2) {
        ...
        } else self.otherImageView.frame = CGRectMake(self.width + self.imageBorder, 0, self.width - self.imageBorder * 2, self.height);
    ...
    //向左滚动
        ...
        } else self.otherImageView.frame = CGRectMake(CGRectGetMaxX(_currImageView.frame) +  2 * self.imageBorder, 0, self.width - 2 * self.imageBorder, self.height);
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    if ([self.delegate respondsToSelector:@selector(carouselViewWillBeginDragging)]){
          [self.delegate carouselViewWillBeginDragging];
      }
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate{
      // 停止类型1、停止类型2
      BOOL scrollToScrollStop = !scrollView.isTracking && !scrollView.isDragging && !scrollView.isDecelerating;
      
            if (scrollToScrollStop) {
                  if ([self.delegate respondsToSelector:@selector(carouselViewDidEndScroll)]){
                       [self.delegate carouselViewDidEndScroll];
                  }
            }
