...
- (void)showInWindowWithBackgroundColorOpacity:(CGFloat)oparcity
{
    [self showInWindowWithBackgoundTapDismissEnable:NO backgroundOpacity:oparcity];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [self hideInWindow];
    });
}

- (void)showInWindowWithBackgroundColorOpacity:(CGFloat)oparcity tapDismissEnable:(BOOL)tapDismissEnable autoDismiss:(BOOL)autoDismiss {
    [self showInWindowWithBackgoundTapDismissEnable:tapDismissEnable backgroundOpacity:oparcity];
    if (autoDismiss) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [self hideInWindow];
        });
...
- (void)showInWindowWithBackgoundTapDismissEnable:(BOOL)backgoundTapDismissEnable backgroundOpacity:(CGFloat)opacity
{
    if (self.superview) {
        [self removeFromSuperview];
    }
    [TYShowAlertView showAlertViewWithView:self backgoundTapDismissEnable:backgoundTapDismissEnable backgroundOpacity:opacity];
}
...
-(void)hideInWindow {
    ...
    [self hideInWindows:true];
}

- (void)hideInWindows:(BOOL) ifSendNot
{
    if ([self isShowInWindow]) {
        [(TYShowAlertView *)self.superview hide:ifSendNot];
    }else {
