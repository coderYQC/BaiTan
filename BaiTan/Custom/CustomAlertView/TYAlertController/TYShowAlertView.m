...
+ (void)showAlertViewWithView:(UIView *)alertView backgoundTapDismissEnable:(BOOL)backgoundTapDismissEnable backgroundOpacity:(CGFloat)opacity
{
    ...
    showTipView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:opacity];
...
- (void)addBackgroundView
{
    if (_backgroundView == nil) {
        ...
        backgroundView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:self.backgroundOpacity];
...
- (void)show
{
    if (self.superview == nil) {
        [kCurrentWindow insertSubview:self atIndex:1];
    }
    self.alpha = 0;
...
- (void)hide:(BOOL) ifSendNot
{
    if (self.superview) {
        [UIView animateWithDuration:0.2 animations:^{
            _alertView.transform =
            CGAffineTransformScale(_alertView.transform,0.1,0.1);
            self.alpha = 0;
        } completion:^(BOOL finished) {
            [self removeFromSuperview];
            if (ifSendNot) {
                [[NSNotificationCenter defaultCenter] postNotificationName:@"KNotificationAlertViewDismissed" object:nil];
            }
        }];
    }
}
